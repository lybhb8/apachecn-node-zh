...
Node-RED 所拥有的许多节点中，适合调用 web api (REST api)的节点并不多。 调用 web API 时使用的一个典型节点是**http request**节点。
...
**http request**节点的进程不能单独由**http request**节点执行。 在**http request**节点之前，有必要连接触发进程，例如**inject**节点。 此时，如果您想传递一个参数给 API 调用，即 **http request** 节点，请在**msg.payload**。

如果您想调用的 API 在 http 请求 **节点**,JSON 数据被包含在请求将由创造满意作为请求参数的预处理节点,将它存储在**味精。 有效负载**，并将其连接到 **http request** 节点。

 通过使用这样的**http request**节点，可以轻松实现 API 协作。 API 调用对于链接 Node-RED 上的多个服务非常重要。 例如，node - red 的**function**节点基本上是由 JavaScript 处理的，但通过将使用其他开发语言(如 Java)开发的程序转换为 API，就可以通过从 node - red 调用来使用它。
...
> 重要提示:在创建 API 密钥之后，密钥将在 10 分钟到几个小时内不会被激活。 如果当您访问下一节中描述的 API 端点 URL 时返回一个 web 响应错误，例如 401，那么指定的 API 键可能还没有被激活，因此请等待并重试。
...
2.  本教程中将使用**Current Weather Data（当前天气数据）**，请点击**Current Weather Data**下方的**API doc**按钮。

![Figure 7.5 – Opening API doc of Current Weather Data ](img/Figure_7.5_B16353.jpg)

    图 7.5 -打开当前天气数据的 API 文档

3.  该 API 包含多种参数类型，例如按城市（By city）、按城市 ID（By city ID）、按邮政编码（By zip code） 等。请选择按城市名称（By city name） 这一类型，并传入城市名称和API 密钥作为参数。
...
1.  在调色板上放置一个**注入**节点和两个**debug**节点。 这些节点可以作为默认节点使用。 这里不需要更改设置。
2.  在面板上放置http request节点，随后打开该http request节点的设置面板，并在面板的URL文本框中，按下图所示填入包含你的参数（城市名称和 API 密钥）的 API 接口地址：

![Figure 7.9 – Setting the API endpoint URL with your parameters ](img/Figure_7.9_B16353.jpg)

    图 7.9 -使用参数设置 API 端点 URL

3.  在面板上放置一个json节点。该节点可直接使用默认配置，此处无需修改任何设置。但为了稳妥起见，我们需确认json节点的Action属性已设置为Convert between JSON String & Object（在 JSON 字符串与对象间转换）。这个选项会将作为输入参数传入的 JSON 数据转换为 JSON 对象avaScript object:
...
请连接时间戳节点和 http 请求节点。**http request** 节点输出连接到**json**节点和**debug**节点。 最后，请将**json**节点输出连接到另一个**debug**节点。
...
你也可以在同一个**debug**窗口中看到 JSON 对象的结果数据，如下截图所示:
...
为什么华生? watson是 IBM 提供的人工智能服务和 api 品牌。

所有watson api 都可以从 IBM Cloud 使用。 因此，通过在 IBM Cloud 上运行 Node-RED，您可以有效地使用 Watson 的服务。 这有一些优点，比如当从 Node-RED 调用 Watson API 时，可以省略身份验证的实现。

watson可以从 IBM Cloud 以外的环境调用，因此它可以直接从树莓派调用，也可以从 AWS 和 Azure 等云平台或本地环境中使用。 请看下图，显示了watson API 的样子:
...
按照以下步骤创建watson音调分析仪 API 服务:

1.  在资源目录中搜索 **Watson**。在仪表板上，点击**Catalog**菜单项，搜索**tone analyzer**，然后选择**Tone Analyzer**面板。:

![Figure 7.15 – Searching Watson services ](img/Figure_7.15_B16353.jpg)

    图 7.15 -搜索 Watson 服务

2.  请参考以下列表及**图7.16**填写各项属性。

    a.**Region地区**:**达拉斯**(您可以选择任何地区，但推荐达拉斯)

    b.**Pricing plan定价方案**:**Lite**(免费定价)

    c.**Service name服务名称**:**默认**(您可以将其修改为您想要使用的任何名称)

    d.**Resource group资源组**:**默认**(Lite 帐号不能选择其他选项)

    e.**Tags标签**:无

3.  填写 / 选择完所有属性后，点击Create按钮：

![ Figure 7.16 – Creating a Tone Analyzer service ](img/Figure_7.16_B16353.jpg)

    图 7.16 -创建音调分析器服务

4.  在**Tone Analyzer**实例仪表板上创建并激活后，你可以看到状态为**Active**。请查看 **API key** 和 **URL**。
API 密钥和 URL 用于从任意应用调用 API，但在本教程中**不会使用**，因为 IBM Cloud 上的 Node‑RED 可以无需认证编码直接调用 Watson API。
...
*   **node-red-node-sentiment**:这是一个在传递的**msg 中添加情绪对象的节点。 有效载荷**。 当将参数传递给watson音调分析仪 API 时使用:
...
6.  点击设置窗口中的**编辑（铅笔图标）**按钮，以编辑你的Twitter信息：
...
12.  在工作区中，将**tone analyzer v3**节点按顺序放置在**sentiment**节点之后：

![Figure 7.35 – Placing the tone analyzer v3 node ](img/Figure_7.35_B16353.jpg)

    图 7.35 -放置音调分析器 v3 节点

13.  打开**tone analyzer v3**节点的设置面板，并按如下所示设置**Method**（方法）和**URL**（URL 地址）属性：

- a. Name（名称）：可自定义任意字符串
- b. Method（方法）：通用语气（General Tone）
- c. version_date（版本日期）：多种语气（Multiple Tones）
- d. Tones（语气类型）：全部（All）
- e. Sentences（句子分析）：开启（True）
- f. Content type（内容类型）：文本（Text）
    
![Figure 7.36 – Configuring the tone analyzer v3 node properties ](img/Figure_7.36_B16353.jpg)

    图 7.36 -配置音调分析器 v3 节点属性

14. 在你的工作区中，将函数（function） 节点按顺序放置在语气分析器 v3（tone analyzer v3） 节点之后：

![Figure 7.37 – Placing the function node ](img/Figure_7.37_B16353.jpg)

    图 7.37 -放置功能节点

15.  打开函数（function） 节点的设置面板，并使用以下源代码编写 JavaScript 代码：
...
16.  最后，在这个**功能**节点之后并行放置三个**debug**节点。 每个**debug**节点的设置如下:
    - **msg.payload.text**:用于**debug**标签
    - **msg.payload.tone**:用于**debug**标签
    - **msg.payload.sentiment**: 用于**debug**标签
...
这里设置为获取所有带有 #nodered 标签的推文。如果获取到的推文数量很少，说明目前还没有包含该指定标签的推文，请修改 Twitter 输入节点中设置的标签后重试。

此流程的所有处理结果将显示在调试标签页中。

从获取的推文中提取推文正文并进行显示的变量是 **msg.payload.text**。

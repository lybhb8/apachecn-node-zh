...
我已经解释了如何开始 Node-RED 覆盆子π,所以你现在应该知道如何运行它,但是如果你需要复习,请参阅[第 2 章](02.md#树莓派安装-node-red):*搭建开发环境中 **为树莓派安装 Node-RED** 一节。*

现在，按照以下步骤在树莓派上启动 Node-RED:

1. 我们先从树莓派菜单中启动 Node‑RED。

   ![Figure 5.1 – Running Node-RED from the Raspberry Pi menu ](img/Figure_5.01_B16353.jpg)

   图 5.1 -在树莓派菜单中运行 Node-RED
2. 你可以在终端中查看 Node‑RED 的状态。如果显示 \*\*Started flows\*\*，则说明 Node‑RED 已准备就绪，可以使用:
...
> 重要提示: 本章给出了一个使用 Grove Base HAT 的例子，它相对便宜，可以购买(链接在下一节中提到)，但是任何可以连接到树莓派的传感器设备都可以处理 Node-RED 上的数据。
...
2. 如果一切顺利，你将会看到如下提示：
   ...
   图 5.4 -成功安装 grove.py
3. 下一步是启用 ARM I2C。我们可以通过执行以下命令来完成：
```bash
   $ sudo raspi-config
```
4. 执行命令后，你会看到如下配置窗口，请选择 **Interfacing Options**:
...
> 重要提示:   如果你没有树莓派的 Grove Base HAT，但想运行本教程，请通过官方网站([https://www.seeedstudio.com/Grove-Base-Hat-for-Raspberry-Pi.html](https://www.seeedstudio.com/Grove-Base-Hat-for-Raspberry-Pi.html))购买。
...
1. 将 Grove Base HAT 扩展板安装到树莓派上并拧紧螺丝：
...
2. 将 Grove 光线传感器连接到扩展板（Base HAT）的**模拟量端口**。

   ![Figure 5.10 – Connecting the light sensor to your Base HAT ](img/Figure_5.10_B16353.jpg)

   图 5.10 -连接光传感器到 Base HAT

> 重要提示：请小心! 这个供应商，**Seeed，**有一个类似的模块用于温度/湿度传感器**SHT35**，但是 Grove Base HAT 节点不支持它。 你需要使用**SHT31**。
...
如果你需要复习一下关于如何安装节点的节点上图书馆出版,请阅读*从图书馆得到几个节点*在[*第四章*](04.md#四学习主要节点),*学习的主要节点。*

*我建议您回到这里的原因是，下一步是将 Grove Base HAT 的节点从库安装到环境中。

让我们在 node - red 流编辑器中启用这个 Grove Base HAT 节点:

1. 点击右上角的菜单，选择 **管理面板（Manage palette）** 打开设置面板：

   ![Figure 5.15 – Selecting Manage palette](img/Figure_5.15_B16353.jpg)

   图 5.15 -选择 Manage 调色板
2. 设置面板打开后，在搜索框中输入你要使用的节点名称。我们要使用 **node-red-contrib-grove-base-hat**，因此请输入以下内容：

```bash
   grove base
```

3. 之后，你就能在搜索窗口中看到 **node-red-contrib-grove-base-hat** 节点。点击 **Install（安装）** 按钮：
...
1. 在流程编辑器左侧的面板中选中 **grove light sensor v1_2** 节点，将其拖拽并放置到工作区中：
...
2. 放置好 **grove-light-sensor-v1_2** 节点后，再放置 **inject**（注入）节点和 **debug**（调试）节点，并进行连线，使你放置的 **grove-light-sensor-v1_2** 节点位于它们中间：

   ![Figure 5.20 – Placing nodes and wiring them for the light sensor ](img/Figure_5.20_B16353.jpg)

   图 5.20 -放置**grove-light-sensor-v1_2**
3. 接下来，检查**grove-light-sensor-v1_2**节点的设置。 双击节点打开设置面板。
4. 设置面板中有一个名为 **Port（端口）** 的选项，默认选中的是 **A0**。

   此**Port**设置指定 Grove Base HAT 上的哪个连接器从所连接的模块获取数据。

5. 之前我们已经将 Grove 光线传感器连接到了 Grove Base HAT 扩展板。
如果按照本教程的步骤进行接线，传感器应连接在 **A2 端口**，因此请在节点设置中选择 **A2**。如果你连接到了其他端口，请选择实际所连接的端口：
...
6. 在设置面板中检查并设置好**端口（Port）**后，点击右上角的**完成（Done）**按钮，关闭设置面板。
   就是这样! 不要忘记单击**deploy**按钮。

您应该记住如何从注入节点执行流，因为您已经在前一章中了解了这一点。 单击注入节点上的开关运行流程。 当开关被点击的时间数据被输出为一个日志，所以请尝试点击它几次。

> 重要提示:  不要忘记显示调试窗口，以显示获取数据的值将输出到调试窗口。 即使激活了调试输出，Node-RED 也不会自动显示调试窗口。
...
1. 在流程编辑器左侧的节点面板中选中 **grove temperature humidity sensor sht3x** 节点，将其拖拽并放置到工作区中：
...
2. 放置好 **grove-temperature-humidity-sensor-sht3x** 节点后，分别放置 **inject**（注入）节点和 **debug**（调试）节点，并进行连线，使你放置的 **grove-temperature-humidity-sensor-sht3x** 节点位于它们中间。
...
3. 接下来，检查 **grove-temperature-humidity-sensor-sht3x** 节点的设置，双击该节点打开设置面板。
...
4. 在设置面板中检查完**端口（Port）**后，点击右上角的**完成（Done）**按钮，关闭设置面板。

   就是这样! 不要忘记单击**deploy**按钮。
5. 点击 **inject 节点**上的开关以运行流程。点击开关时的时序数据会作为日志输出，你可以试着点击几次。

> 重要提示:  如前一节所述，不要忘记显示调试窗口，以显示获取的数据的值将是调试窗口的输出。 即使激活了调试输出，Node-RED 也不会自动显示调试窗口。

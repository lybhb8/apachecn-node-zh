...
现在，您已经准备好使用 Node-RED 进行编程了。 从[*第三章*](03.md#三通过创建基本流来理解-node-red-特性)，*通过创建基本流程来理解 Node-RED 特性*，接下来将学习如何使用 Node-RED 实际构建应用。
...
现在，您已经准备好使用 Node-RED 进行编程了。 在[*第三章*](03.md#三通过创建基本流来理解-node-red-特性)，*通过创建基本流程来理解 Node-RED 特性，*我们将学习如何使用 Node-RED 实际构建应用。
...
1.  通过命令行运行：若需在本地运行Node-RED，可通过终端输入node-red命令启动。随后按下Ctrl + C或关闭终端窗口即可停止运行。
显示效果如下:
